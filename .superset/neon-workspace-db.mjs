...
async function setup() {
  ...
  const workspaceName = process.env.SUPERSET_WORKSPACE_NAME || basename(workspaceDir)
  ...
  if (!existsSync(envPath)) {
    if (rootPath && existsSync(resolve(rootPath, '.env'))) {
      ...
      console.error('ERROR: No .env file found. Copy root .env first or set SUPERSET_ROOT_PATH.')
      process.exit(1)
    }
  }
  ...
  const metadataDir = resolve(workspaceDir, '.superset', 'context')
...
async function teardown() {
  const workspaceDir = process.cwd()
  const metadataPath = resolve(workspaceDir, '.superset', 'context', 'neon-workspace.json')
  ...
  if (!projectId || !apiKey) {
    console.error('ERROR: NEON_PROJECT_ID and NEON_API_KEY are required for teardown.')
    process.exit(1)
  }
...
if (command === 'setup') {
  ...
  console.error('Usage: node neon-workspace-db.mjs <setup|teardown>')
  process.exit(1)
}
